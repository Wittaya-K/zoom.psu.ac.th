...
@section('content')
    <h3 class="page-title">คำขอใช้งานบัญชี</h3>
    @can('booking_create')
    <p>
        <a href="{{ route('admin.bookings.create') }}" class="btn btn-success">จองบัญชี</a>
                    ...
                    @if (count($bookings) > 0)
                        @foreach ($bookings as $booking)
                            <tr data-entry-id="{{ $booking->id }}">
                                ...
                                <td field-key='customer'>{{ $booking->user_name or '' }}</td>
                                <td field-key='room'>{{ $booking->room_number or '' }}</td>
                                ...
                                @if($booking->name == 'ว่าง')
                                <td field-key='additional_information'><small class="label pull-center bg-green">{{ $booking->name }}</small></td>
                                @else
                                <td field-key='additional_information'><small class="label pull-center bg-red">{{ $booking->name }}</small></td>
                                @endif
                                ...
                                <td> 
                                    ...
                                    @endcan
                                    @can('booking_edit')
                                    <a href="{{ route('admin.bookings.edit',[$booking->id]) }}" class="btn btn-xs btn-warning">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('booking_delete')
{!! Form::open(array(
                                        'style' => 'display: inline-block;',
