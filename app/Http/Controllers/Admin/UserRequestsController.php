...
class UserRequestsController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request('show_deleted') == 1) {
            ...
            if(auth()->user()->name == 'Admin') //เช็คสิทธิ์ Admin
            {
                ...
                // ->select('bookings.id','bookings.user_name','bookings.time_from','bookings.time_to','bookings.additional_information','rooms.id','rooms.room_number','rooms.category_id','categories.name')
                ->leftJoin('rooms', 'rooms.id', '=', 'bookings.id')
                ->leftJoin('categories', 'categories.id', '=', 'rooms.category_id')
                ->get();
            }else
            {
                $bookings = DB::table('bookings')->where('user_name', '=', auth()->user()->name)->get();
            }
    ...
    public function store(StoreBookingsRequest $request)
    {
        ...
        DB::insert('insert into bookings(time_from, time_to, additional_information, created_at, updated_at, deleted_at, customer_id, room_id, user_name) values(?,?,?,?,?,?,?,?,?)',array($time_from,$time_to,$additional_information,now(),null,null,$customer_id,$room_id,$user_name));
