...
class AuthServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Auth gates for: User management //กำหนดสิทธิ์ใช้งานเมนู
        ...
        // Auth gates for: Zooms
        Gate::define('zoom_access', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('zoom_create', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('zoom_edit', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('zoom_view', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('zoom_delete', function ($user) {
            return in_array($user->role_id, [1]);
        });
        ...
        // Auth gates for: Bookings
        Gate::define('booklists_access', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });
        Gate::define('booklists_delete', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });
